...
export function SignupChoiceCard() {
  ...
  return (
    <section className="signup-exact__container" aria-labelledby="signup-title">
      <button type="button" className="signup-exact__close" aria-label="Fermer" title="Fermer" onClick={closeSignup}>
        <CloseIcon />
      </button>
        ...
        {accountCards.map((card) => (
          <article key={card.key} className={`signup-exact__card signup-exact__card-${card.key}`}>
            ...
            <div className="signup-exact__card-actions">
              <a href={card.href} className="signup-exact__circle-arrow" aria-label={`Choisir ${card.badge.toLowerCase()}`}>
                <ArrowRightIcon />
              </a>
            </div>
          </article>
