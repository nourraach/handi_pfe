...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'type_notification') THEN
    CREATE TYPE type_notification AS ENUM ('application', 'message', 'interview', 'system');
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS offre_publication_review (
  ...
  reviewed_by UUID REFERENCES utilisateur(id_utilisateur) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_offre_emploi_entreprise ON offre_emploi(id_entreprise);
CREATE INDEX IF NOT EXISTS idx_offre_emploi_statut ON offre_emploi(statut);
CREATE INDEX IF NOT EXISTS idx_favoris_candidat ON favoris(id_candidat);
CREATE INDEX IF NOT EXISTS idx_recommendation_candidat ON recommendation(id_candidat);
