...
export async function openSupervisionReportPdf(reportId: string) {
  const opened = window.open("", "_blank");
  if (!opened) {
    throw new Error("Impossible d'ouvrir le PDF. Autorisez les fenêtres contextuelles puis réessayez.");
  }

  const response = await authenticatedFetch(construireUrlApi(`/api/supervision/reports/${reportId}/pdf`));
  if (!response.ok) {
    opened.close();
    throw new Error("Unable to open the compliance report PDF.");
  }
