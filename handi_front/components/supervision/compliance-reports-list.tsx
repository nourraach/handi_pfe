...
import { ComplianceReportSummary, openSupervisionReportPdf } from "@/lib/supervision";
...
export function ComplianceReportsList() {
  ...
  const openPdf = async (report: ComplianceReportSummary) => {
    try {
      setPdfError(null);
      setOpeningReportId(report.id);
      await openSupervisionReportPdf(report.id);
    } catch (error) {
      setPdfError(error instanceof Error ? error.message : "Impossible d'ouvrir le PDF du rapport.");
    } finally {
  ...
  return (
    <SupervisionShell>
      <section className={styles.sectionStack}>
        {pdfError ? <div className="rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-rose-800">{pdfError}</div> : null}
              ...
              <article key={report.id} className={styles.reportCard}>
                ...
                <div className={styles.cardFooter}>
                  ...
                  <div className={styles.reportActions}>
                    ...
                    >
                      <Download size={15} />
                      {openingReportId === report.id ? "Ouverture..." : "Voir le PDF"}
                    </Button>
                  </div>
                </div>
              </article>
