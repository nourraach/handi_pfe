...
const formatContractLabel = (typePoste: string) => {
  const normalized = typePoste.trim().toUpperCase();
...
const frenchTextReplacements: Array<[RegExp, string]> = [
  ...
  [/\bCharge\(e\)\b/g, "Chargé(e)"],
  [/\bcharge\b/gi, "chargé"],
  [/\bConfirm[eé]\b/g, "Confirmé"],
  [/\bPresentiel\b/gi, "Présentiel"],
  [/\bteletravail\b/gi, "télétravail"],
  [/\bsante\b/gi, "santé"],
  [/\be-sante\b/gi, "e-santé"],
  ...
  [/\bpubliee\b/gi, "publiée"],
  [/\bbientot\b/gi, "bientôt"],
  [/\bElargissez\b/g, "Élargissez"],
  [/\bElargir\b/g, "Élargir"],
  [/\bdetails\b/gi, "détails"],
  [/\bDeja\b/g, "Déjà"],
  [/\bpostule\b/gi, "postulé"],
  [/\brenseignee\b/gi, "renseignée"],
  [/\brenseigne\b/gi, "renseigné"],
  [/\bCompetences\b/g, "Compétences"],
  [/\bcompetences\b/gi, "compétences"],
  [/\brecherchees\b/gi, "recherchées"],
  [/\bgenerer\b/gi, "générer"],
  [/\bresume\b/gi, "résumé"],
  [/\bCite\b/g, "Cité"],
  [/\bRades\b/g, "Radès"],
  [/\bMegrine\b/g, "Mégrine"],
  [/\bben arous\b/gi, "Ben Arous"],
  [/(\d+)\s+a\s+(\d+)\s+ans/gi, "$1 à $2 ans"],
];

const formatFrenchDisplayText = (value?: string | null, fallback = "") => {
  let text = value?.trim() || fallback;
  frenchTextReplacements.forEach(([pattern, replacement]) => {
...
const formatExperienceLabel = (experience?: string) => {
  ...
  return formatFrenchDisplayText(experience);
};
...
const getOfferTitle = (offre: OffreEmploi) => formatFrenchDisplayText(offre.titre);
const getOfferLocation = (offre: OffreEmploi) => formatFrenchDisplayText(offre.localisation || "Tunis");
const getOfferCompany = (offre: OffreEmploi) => formatFrenchDisplayText(offre.nom_entreprise || "Entreprise de test");
const getOfferDescription = (offre: OffreEmploi) => formatFrenchDisplayText(offre.description);
...
const buildOfferSpeechContent = (offre: OffreEmploi, aiSummary?: string) => {
  const company = getOfferCompany(offre) || "Non précisé";
  const skills = formatFrenchDisplayText(offre.competences_requises, "Non précisé");
  const experience = formatFrenchDisplayText(offre.experience_requise, "Non précisé");
  const location = getOfferLocation(offre) || "Non précisé";
  const contractType = formatContractLabel(offre.type_poste) || "Non précisé";
  const description = getOfferDescription(offre) || "Non précisé";
  const summary = formatFrenchDisplayText(aiSummary, "Non précisé");

  return [
    `Titre du poste: ${getOfferTitle(offre)}`,
    `Entreprise: ${company}`,
...
export default function OffresPage() {
  ...
  const postuler = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.status === 409) {
        setInfo(data.message || "Vous avez déjà postulé à cette offre.");
        setCandidatures((prev) => new Set(prev).add(offreSelectionnee.id_offre));
        reinitialiserFormulaireCandidature();
        return;
      }
      ...
      <section className="jobs-studio-shell" aria-busy={loading} aria-live="polite">
            ...
            >
              ...
              {locations.map((location) => (
                <option key={location} value={location}>
                  {formatFrenchDisplayText(location)}
                </option>
              ))}
            </select>
          </label>
        ...
        {offresFiltered.length === 0 ? (
          <div className="jobs-studio-empty" aria-live="polite">
            ...
            <p>{hasActiveFilters ? "Élargissez la recherche pour découvrir plus d'opportunités." : "Aucune offre n'est publiée pour le moment, revenez bientôt."}</p>
            {hasActiveFilters ? (
          ...
          <div className="jobs-studio-grid is-list">
            {offresVisibles.map((offre) => {
              ...
              const titreOffre = getOfferTitle(offre);
              const entrepriseOffre = getOfferCompany(offre);
              const localisationOffre = getOfferLocation(offre);
              ...
              return (
                <article key={offre.id_offre} className="jobs-simple-card" tabIndex={0}>
                  <div className="jobs-simple-card__top">
                    <div className="jobs-simple-card__topMain">
                      <div className="jobs-simple-card__avatar" aria-hidden="true">
                        {(titreOffre.match(/[A-Za-zÀ-ÖØ-öø-ÿ]/g) || ["U", "X"]).slice(0, 2).join("").toUpperCase()}
                      </div>
                      <div className="jobs-simple-card__headerText">
                        <h2 className="jobs-simple-card__title" title={titreOffre}>
                          {titreOffre}
                        </h2>
                        <p className="jobs-simple-card__company" title={entrepriseOffre}>
                          {entrepriseOffre}
                        </p>
                      </div>
                    </div>
                    <button
                      ...
                      aria-label={`${estFavori ? "Retirer" : "Sauvegarder"} l'offre ${titreOffre}`}
                    >
                      <BookmarkIcon />
                    </button>
                  </div>
                  ...
                  <div className="jobs-simple-card__main">
                    <p className="jobs-simple-card__location" title={localisationOffre}>
                      {localisationOffre}
                    </p>
                    ...
                    <div className="jobs-simple-card__tags">
                      <span className="jobs-simple-card__pill jobs-simple-card__pill--access">Accessibilité adaptée</span>
                  ...
                  <div className="jobs-simple-card__footer">
                    <button
                      ...
                      aria-label={`Voir les détails de l'offre ${titreOffre}`}
                    >
                      Voir les détails
                    </button>
                    <button
                      ...
                      aria-label={`Postuler à l'offre ${titreOffre}`}
          ...
          >
            ...
            <div className="company-job-profile">
              ...
              <section className="company-job-hero">
                <div className="company-job-logo">{getOfferCompany(offreEnDetails).slice(0, 2).toUpperCase()}</div>
                <div>
                  <p className="badge jobs-modal-eyebrow">Entreprise qui recrute</p>
                  <h2 id="job-details-modal-title" className="jobs-modal-title">
                    {getOfferCompany(offreEnDetails)}
                  </h2>
                  <p className="jobs-modal-subtitle">
                    {getOfferTitle(offreEnDetails)} - {getOfferLocation(offreEnDetails)}
                  </p>
                  <div className="company-job-meta">
                    <span><LocationIcon /> {getOfferLocation(offreEnDetails)}</span>
                    <span><ExperienceIcon /> {formatContractLabel(offreEnDetails.type_poste)}</span>
              ...
              <section className="company-job-kpis">
                <article><ExperienceIcon /><span>Poste</span><strong>{getOfferTitle(offreEnDetails)}</strong></article>
              ...
              <section className="company-job-grid">
                <article className="company-job-panel company-job-panel-wide">
                  ...
                  <p>{getOfferDescription(offreEnDetails)}</p>
                </article>
              ...
              <Card tone="accent" padding="md">
                <div className="stack-lg">
                  ...
                  {summaryByOfferId[offreEnDetails.id_offre] ? (
                    ...
                    <p className="texte-secondaire jobs-summary-helper">
                      Cliquez sur le bouton pour générer un résumé simple et accessible.
                    </p>
                  )}
                </div>
              </Card>
                ...
                <article className="company-job-panel">
                  ...
                  <div className="company-job-skills">
                    {(formatFrenchDisplayText(offreEnDetails.competences_requises, "Communication, Adaptabilité, Collaboration"))
                      .split(/[,;•]/)
                ...
                <article className="company-job-panel">
                  ...
                  <div className="company-job-stack">
                    <span><b>Expérience</b>{formatFrenchDisplayText(offreEnDetails.experience_requise, "Non renseignée")}</span>
                    <span><b>Niveau d&apos;étude</b>{formatFrenchDisplayText(offreEnDetails.niveau_etude, "Non renseigné")}</span>
                    <span><b>Accessibilité</b>Processus inclusif HandiTalents</span>
                  </div>
                </article>
              </section>
  ...
  if (loading) {
    return utilisateur ? (
      <AppShell utilisateur={utilisateur}>
        <LoadingState title="Chargement des offres" description="Préparation d'une expérience fluide pour votre recherche." />
      </AppShell>
    ) : (
