...
type OffreEntreprise = {
  ...
  publication_review_status?: "pending" | "approved" | "rejected";
...
function getStatusBadgeConfig(offre: OffreEntreprise) {
  if (offre.statut === "active") {
    return { label: "Published", className: "status-active" };
  }

  if (offre.publication_review_status === "pending") {
    return { label: "Pending review", className: "status-pending" };
  }

  if (offre.publication_review_status === "rejected") {
  ...
  const creerNouvelleOffre = async (nouvelleOffre: OffreFormulaire) => {
    try {
      ...
      if (response.ok) {
        ...
        setMessage("Role created and sent for admin review.");
        void chargerOffres();
        return;
      }
      ...
      if (response.ok) {
        setMessage(
          nouveauStatut === "active"
            ? "Publication request sent to admin."
      ...
      {offresFiltrees.length === 0 ? (
        ...
        <section className="roles-list">
          {offresPage.map((offre) => {
            const status = getStatusBadgeConfig(offre);
            const isActive = offre.statut === "active";
            const canRequestPublication = !["active", "expiree", "pourvue"].includes(offre.statut);
            return (
              <article className="role-card" key={offre.id_offre}>
                ...
                <div className="role-main">
                  ...
                  {offre.publication_review_status === "rejected" && offre.publication_rejection_reason ? (
                    <p className="role-review-reason">Reason: {offre.publication_rejection_reason}</p>
                  ) : null}
                  ...
                  <div className="actions-row">
                    ...
                    {canRequestPublication ? (
                      <Button
                        variant="primary"
                        size="sm"
                        disabled={offre.publication_review_status === "pending"}
                        onClick={() => changerStatut(offre.id_offre, "active")}
                      >
                        {offre.publication_review_status === "pending"
                          ? "Pending admin review"
                          : offre.publication_review_status === "rejected"
                            ? "Resubmit for review"
                            : "Request publication"}
                      </Button>
                    ) : (
