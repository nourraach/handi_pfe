...
const STATUTS_ELIGIBLES_BIEN_ETRE = new Set(["planifie", "confirme", "reporte"]);

function estEligibleBienEtre(dateHeure: string, statut: string) {
  if (!STATUTS_ELIGIBLES_BIEN_ETRE.has(statut)) {
    return false;
  }

  const dateEntretien = new Date(dateHeure);
  if (Number.isNaN(dateEntretien.getTime())) {
    return false;
  }

  const maintenant = Date.now();
  const diffMs = dateEntretien.getTime() - maintenant;
  const fenetreMs = 36 * 60 * 60 * 1000;
  return diffMs >= 0 && diffMs <= fenetreMs;
}
              ...
              <div className="entretien-stack">
                <div className="entretien-head">
                  ...
                  <div className="entretien-head-actions">
                    ...
                    {estEligibleBienEtre(item.entretien.date_heure, item.entretien.statut) ? (
                      <ButtonLink href={`/candidat/entretiens/${item.entretien.id}/bien-etre`} variant="secondary">
                        Preparation 5 min
                      </ButtonLink>
                    ) : null}
                ...
                <div className="entretien-actions">
                  {estEligibleBienEtre(item.entretien.date_heure, item.entretien.statut) ? (
                    <ButtonLink href={`/candidat/entretiens/${item.entretien.id}/bien-etre`}>
                      Preparation 5 min
                    </ButtonLink>
                  ) : null}
