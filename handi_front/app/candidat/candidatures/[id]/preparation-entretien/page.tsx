...
async function lireReponseApi<T>(response: Response): Promise<ApiResponse<T>> {
  const texte = await response.text();
  if (!texte.trim()) return {};

  try {
    return JSON.parse(texte) as ApiResponse<T>;
  } catch {
    if (!response.ok) {
      throw new Error("Le service de préparation est momentanément indisponible. Réessayez dans quelques secondes.");
    }
    throw new Error("La réponse du service de préparation est invalide.");
  }
}
...
export default function PreparationEntretienPage() {
  ...
  const charger = useCallback(async () => {
    ...
    try {
      ...
      const data = await lireReponseApi<DossierView>(response);
      if (!response.ok) {
        throw new Error(data?.message || "Impossible de charger le dossier.");
      }
      if (!data.donnees) {
        throw new Error("Le dossier de préparation est introuvable.");
      }
      setDossier(data.donnees);
    } catch (e: unknown) {
  ...
  const hasVisibleDossier = dossier.questions.length > 0;
  const isRefreshingDossier = hasVisibleDossier && (dossier.status === "pending" || dossier.status === "processing");
  const regenerationButtonLabel = regenerating
    ? "Régénération..."
    : isRefreshingDossier
      ? "Mise à jour en cours"
      : dossier.can_regenerate
        ? "Régénérer (1 fois max)"
        : "Régénération déjà utilisée";
  ...
  if ((dossier.status === "pending" || dossier.status === "processing") && !hasVisibleDossier) {
    return (
      <main className="mx-auto w-full max-w-6xl px-4 py-6 lg:px-6">
  ...
  if (dossier.status === "failed" && !hasVisibleDossier) {
    return (
      <main className="mx-auto w-full max-w-6xl px-4 py-6 lg:px-6">
        <Card className="p-6">
          ...
          <Card className="p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              ...
              <div className="rounded-2xl bg-gray-50 px-4 py-3">
                ...
                <p className="mt-1 text-sm font-semibold text-gray-900">{isRefreshingDossier ? "Mise à jour" : "Prêt"}</p>
              </div>
            </div>
          </Card>
          ...
          {dossier.source === "fallback" ? (
            <Card className="border-amber-100 bg-amber-50 p-5">
              ...
              <Button
                ...
                disabled={!dossier.can_regenerate || regenerating}
              >
                {regenerating ? "Régénération..." : isRefreshingDossier ? "Mise à jour en cours" : "Tenter une version IA"}
              </Button>
            </Card>
          ) : null}
